...
export default function StandardLoginGateway({ onAuthSuccess }) {
    ...
    return (
        <div style={{ maxWidth: '380px', width: '100%', padding: '30px', background: '#fff', borderRadius: '8px', fontFamily: 'sans-serif', boxSizing: 'border-box' }}>
            <h3 style={{ textAlign: 'center', margin: '0 0 20px 0', textTransform: 'capitalize', color: '#1e3a8a', fontSize: '20px' }}>
                Voter {view === 'verify-reset' ? 'Verification' : view} Gateway
            </h3>

            {message.text && (
                <div style={{ padding: '10px', marginBottom: '15px', borderRadius: '4px', fontSize: '13px', lineHeight: '1.4', backgroundColor: message.type === 'error' ? '#fee2e2' : '#d1fae5', color: message.type === 'error' ? '#991b1b' : '#065f46', border: message.type === 'error' ? '1px solid #fca5a5' : '1px solid #a7f3d0', boxSizing: 'border-box' }}>
                    {message.text}
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} style={{ width: '100%', boxSizing: 'border-box' }}>
                ...
                {view === 'register' && (
                    <div style={{ marginBottom: '15px', width: '100%', boxSizing: 'border-box' }}>
                        <label style={{ display: 'block', marginBottom: '6px', fontWeight: 'bold', fontSize: '13px', color: '#334155' }}>Mobile Phone Number (Required for Voting Security)</label>
                        <input type="tel" required placeholder="303-555-0199" style={{ width: '100%', padding: '10px', boxSizing: 'border-box', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '14px' }} value={phone} onChange={(e) => setPhone(e.target.value)} />
                    </div>
                )}
            ...
            <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'space-between', fontSize: '13px', borderTop: '1px solid #f1f5f9', paddingTop: '15px', boxSizing: 'border-box' }}>
                ...
                {view !== 'register' && <span style={{ color: '#0070f3', cursor: 'pointer', fontWeight: '500', textDecoration: 'underline' }} onClick={() => handleSwitchView('register')}>Register Account</span>}
